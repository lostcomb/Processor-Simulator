...
assignDeclParser =   (do Token.reserved lexer "Int"
                         ...
                         aexp  <- aexpParser
                         return $ AssignDeclInt ident aexp
                     )
                 <|> (do Token.reserved lexer "Bool"
                         ident <- Token.identifier lexer
                         Token.reservedOp lexer "="
                         bexp  <- bexpParser
                         return $ AssignDeclBool ident bexp
                     )


indexParser :: Parser (Maybe Int)
indexParser = (   (liftM (Just . fromIntegral) $ Token.brackets lexer (Token.integer lexer))
              <|> (return Nothing                                                          )
              )

exprParser :: Parser (Maybe Aexp)
exprParser = (   (liftM Just $ Token.brackets lexer aexpParser)
             <|> (return Nothing                              )
             )
...
assignParser = do ident <- Token.identifier lexer
                  expr  <- exprParser
                  Token.reservedOp lexer "="
                  ((bexpParser >>= (\bexp -> return $ AssignBexp ident expr bexp)) <|>
                   (aexpParser >>= (\aexp -> return $ AssignAexp ident expr aexp)))
...
aexpTerm =   liftM (Const . fromIntegral) (Token.integer lexer)
         <|> try (liftM Func funcCallParser)
         <|> Var <$> Token.identifier lexer <*> exprParser
         <|> try (do ident <- Token.identifier lexer
                     expr  <- exprParser
